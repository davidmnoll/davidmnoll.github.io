...
I'm also starting to like the idea of attaching a small "eval" hook to the scope itself. Each scope can optionally point at a function that runs every time a new object is added. The default is just the identity function (which keeps the existing behavior of dropping the object into scope untouched), but custom evaluators can rewrite the object, annotate it with scope context, or even normalize it into a different structure before it becomes visible. That gives scopes their own lightweight semantics without forcing every node to understand evaluation rules for every other node.
